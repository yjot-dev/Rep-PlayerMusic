package com.yjotdev.playermusic.data.service
...
class MusicService : Service() {
    @Inject lateinit var stateHolder: PlayerStateRepositoryImpl
        ...
        when (intent?.action) {
            "PLAY" -> {
                val track = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    intent.getParcelableExtra("TRACK", MusicModel::class.java)
                } else {
                    intent.getParcelableExtra("TRACK")
                }
                val playlist = intent.getParcelableArrayListExtra<MusicModel>("PLAYLIST")
    ...
    private fun getPreviousTrack(): MusicModel? {
    ...
    private fun getNextTrack(): MusicModel? {
        if (currentPlaylist.isEmpty()) return null
    ...
    private fun playTrack(track: MusicModel) {
    ...
    private fun startProgressUpdates(track: MusicModel) {
        stopProgressUpdates()
